...
class CLPatchMatch:
    ...
    def __init__(self):
        ...
        self.ctx = cl.Context([self.devices[2]])
    ...
    def loadImages(self,files):
        ...
        self.img = [skimage.img_as_float(files[i]) for i in (0,1)]
        self.loadProgram("patchmatch/patchmatch.c")
        self.times["init"]+=getTime()-t
    ...
    def randomfill(self):
        ...
        self.program.randomfill(self.queue, self.size, None,
    ...
    def execute(self):
        ...
        self.program.propagate(self.queue, self.size, None,
                                  numpy.int32(self.patchSize[0]), #patchHeight
                                  numpy.int32(self.patchSize[1]), #patchWidth
                                  numpy.int32(self.size[0]), #height
                                  numpy.int32(self.size[1]), #width
    ...
    def show(self,nffs=True):
        ...
        for i in range(0,samples):
            ...
            randomPoint=[(int)(random.random()*i) for i in self.size]
    ...
    def match(self,files,patchSize=(5,5),iterations=30,Demo=False):
        ...
        # padSize = (patchSize[0]/2, patchSize[0]/2)
        # files = [numpy.lib.pad(files[i], padSize, 'edge')[:,:,:3] for i in (0,1)]
        self.loadImages(files)

        self.size=self.img[0].shape
        self.patchSize=patchSize
        self.nff=numpy.ndarray((self.size[0],self.size[1],3))

        self.randomfill()

        EPSILON = 0.0005 # if less than 0.05% change, then terminate
        lastMeanDifference = None
        for i in range(0,iterations):
            self.iteration=i+1
            self.execute()
            meanDifference = self.nff[:,:,2].mean()
            if (Demo):
                print "iteration", self.iteration
                print "mean block difference:", meanDifference
            if lastMeanDifference is not None:
                absChange = abs(meanDifference - lastMeanDifference)
                percentChange = absChange/lastMeanDifference
                if percentChange < EPSILON:
                    break
            lastMeanDifference = meanDifference
